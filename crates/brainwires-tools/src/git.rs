...
impl GitTool {
    ...
    fn git_log_tool() -> Tool {
        ...
        properties.insert("limit".to_string(), json!({"type": "number", "description": "Number of commits", "default": 10}));
        Tool {
    ...
    fn git_commit_tool() -> Tool {
        let mut properties = HashMap::new();
        properties.insert("message".to_string(), json!({"type": "string", "description": "Commit message"}));
    ...
    fn git_push_tool() -> Tool {
        ...
        properties.insert("branch".to_string(), json!({"type": "string", "description": "Branch to push"}));
    ...
    fn git_pull_tool() -> Tool {
        let mut properties = HashMap::new();
        properties.insert("remote".to_string(), json!({"type": "string", "description": "Remote name (default: origin)", "default": "origin"}));
        properties.insert("branch".to_string(), json!({"type": "string", "description": "Branch to pull"}));
    ...
    fn git_fetch_tool() -> Tool {
        let mut properties = HashMap::new();
        properties.insert("remote".to_string(), json!({"type": "string", "description": "Remote name (default: origin)", "default": "origin"}));
        properties.insert("all".to_string(), json!({"type": "boolean", "description": "Fetch all remotes", "default": false}));
    ...
    fn git_branch_tool() -> Tool {
        let mut properties = HashMap::new();
        properties.insert("name".to_string(), json!({"type": "string", "description": "Branch name"}));
        ...
        properties.insert("force".to_string(), json!({"type": "boolean", "description": "Force the action", "default": false}));
        Tool {
    ...
    pub fn execute(tool_use_id: &str, tool_name: &str, input: &Value, context: &ToolContext) -> ToolResult {
        ...
        match result {
            ...
            Err(e) => ToolResult::error(tool_use_id.to_string(), format!("Git operation failed: {}", e)),
        }
    }
    ...
    fn git_log(input: &Value, context: &ToolContext) -> Result<String> {
        #[derive(Deserialize)]
        struct Input { #[serde(default = "default_limit")] limit: usize }
        ...
        for (i, oid) in revwalk.enumerate() {
            ...
            output.push_str(&format!("{} - {}\n", commit.id(), commit.summary().unwrap_or("No message")));
        }
        Ok(output)
    }
    ...
    fn git_stage(input: &Value, context: &ToolContext) -> Result<String> {
        #[derive(Deserialize)]
        struct Input { files: Vec<String> }
        let params: Input = serde_json::from_value(input.clone())?;
        let mut cmd = std::process::Command::new("git");
        cmd.current_dir(&context.working_directory).arg("add");
        for file in &params.files { cmd.arg(file); }
        let output = cmd.output()?;
        if output.status.success() {
            Ok(format!("Successfully staged {} file(s)", params.files.len()))
        } else {
            Err(anyhow::anyhow!("Failed to stage files: {}", String::from_utf8_lossy(&output.stderr)))
        }
    }
    ...
    fn git_unstage(input: &Value, context: &ToolContext) -> Result<String> {
        ...
        let params: Input = serde_json::from_value(input.clone())?;
        let mut cmd = std::process::Command::new("git");
        cmd.current_dir(&context.working_directory).args(["reset", "HEAD", "--"]);
        for file in &params.files { cmd.arg(file); }
        let output = cmd.output()?;
        if output.status.success() {
            Ok(format!("Successfully unstaged {} file(s)", params.files.len()))
        } else {
            Err(anyhow::anyhow!("Failed to unstage files: {}", String::from_utf8_lossy(&output.stderr)))
        }
    }

    fn git_commit(input: &Value, context: &ToolContext) -> Result<String> {
        #[derive(Deserialize)]
        struct Input { message: String, #[serde(default)] all: bool }
        let params: Input = serde_json::from_value(input.clone())?;
        let mut cmd = std::process::Command::new("git");
        cmd.current_dir(&context.working_directory).arg("commit");
        if params.all { cmd.arg("-a"); }
        cmd.args(["-m", &params.message]);
        let output = cmd.output()?;
        if output.status.success() {
            Ok(format!("Commit successful:\n{}", String::from_utf8_lossy(&output.stdout)))
        } else {
            Err(anyhow::anyhow!("Commit failed: {}", String::from_utf8_lossy(&output.stderr)))
        }
    }

    fn git_push(input: &Value, context: &ToolContext) -> Result<String> {
        #[derive(Deserialize)]
        struct Input { #[serde(default = "dr")] remote: String, branch: Option<String>, #[serde(default)] set_upstream: bool }
        fn dr() -> String { "origin".to_string() }
        let params: Input = serde_json::from_value(input.clone()).unwrap_or(Input { remote: "origin".to_string(), branch: None, set_upstream: false });
        let mut cmd = std::process::Command::new("git");
        cmd.current_dir(&context.working_directory).arg("push");
        if params.set_upstream { cmd.arg("-u"); }
        cmd.arg(&params.remote);
        if let Some(ref branch) = params.branch { cmd.arg(branch); }
        let output = cmd.output()?;
        if output.status.success() {
            Ok(format!("Push successful:\n{}{}", String::from_utf8_lossy(&output.stdout), String::from_utf8_lossy(&output.stderr)))
        } else {
            Err(anyhow::anyhow!("Push failed: {}", String::from_utf8_lossy(&output.stderr)))
        }
    }

    fn git_pull(input: &Value, context: &ToolContext) -> Result<String> {
        #[derive(Deserialize)]
        struct Input { #[serde(default = "dr")] remote: String, branch: Option<String>, #[serde(default)] rebase: bool }
        fn dr() -> String { "origin".to_string() }
        let params: Input = serde_json::from_value(input.clone()).unwrap_or(Input { remote: "origin".to_string(), branch: None, rebase: false });
        let mut cmd = std::process::Command::new("git");
        cmd.current_dir(&context.working_directory).arg("pull");
        if params.rebase { cmd.arg("--rebase"); }
        cmd.arg(&params.remote);
        if let Some(ref branch) = params.branch { cmd.arg(branch); }
        let output = cmd.output()?;
        if output.status.success() {
            ...
            Err(anyhow::anyhow!("Pull failed: {}", String::from_utf8_lossy(&output.stderr)))
        }
    }

    fn git_fetch(input: &Value, context: &ToolContext) -> Result<String> {
        #[derive(Deserialize)]
        struct Input { #[serde(default = "dr")] remote: String, #[serde(default)] all: bool, #[serde(default)] prune: bool }
        fn dr() -> String { "origin".to_string() }
        let params: Input = serde_json::from_value(input.clone()).unwrap_or(Input { remote: "origin".to_string(), all: false, prune: false });
        let mut cmd = std::process::Command::new("git");
        cmd.current_dir(&context.working_directory).arg("fetch");
        if params.all { cmd.arg("--all"); } else { cmd.arg(&params.remote); }
        if params.prune { cmd.arg("--prune"); }
        let output = cmd.output()?;
        if output.status.success() {
            let stdout = String::from_utf8_lossy(&output.stdout);
            let stderr = String::from_utf8_lossy(&output.stderr);
            let fetch_output = if stdout.is_empty() && stderr.is_empty() { "Already up to date.".to_string() } else { format!("{}{}", stdout, stderr) };
            Ok(format!("Fetch successful:\n{}", fetch_output))
        } else {
            Err(anyhow::anyhow!("Fetch failed: {}", String::from_utf8_lossy(&output.stderr)))
        }
    }
    ...
    fn git_discard(input: &Value, context: &ToolContext) -> Result<String> {
        ...
        let params: Input = serde_json::from_value(input.clone())?;
        let mut cmd = std::process::Command::new("git");
        cmd.current_dir(&context.working_directory).args(["checkout", "--"]);
        for file in &params.files { cmd.arg(file); }
        let output = cmd.output()?;
        if output.status.success() {
            Ok(format!("Successfully discarded changes to {} file(s)", params.files.len()))
        } else {
            Err(anyhow::anyhow!("Failed to discard changes: {}", String::from_utf8_lossy(&output.stderr)))
        }
    }

    fn git_branch(input: &Value, context: &ToolContext) -> Result<String> {
        #[derive(Deserialize)]
        struct Input { name: Option<String>, #[serde(default = "da")] action: String, #[serde(default)] force: bool }
        fn da() -> String { "list".to_string() }
        let params: Input = serde_json::from_value(input.clone()).unwrap_or(Input { name: None, action: "list".to_string(), force: false });
        let branch_name = params.name.clone();
        let mut cmd = std::process::Command::new("git");
        cmd.current_dir(&context.working_directory);
        match params.action.as_str() {
            "list" => { cmd.args(["branch", "-a", "-v"]); }
            "create" => { let n = params.name.ok_or_else(|| anyhow::anyhow!("Branch name required"))?; cmd.args(["branch", &n]); }
            "switch" => { let n = params.name.ok_or_else(|| anyhow::anyhow!("Branch name required"))?; cmd.args(["checkout", &n]); }
            "delete" => {
                let n = params.name.ok_or_else(|| anyhow::anyhow!("Branch name required"))?;
                if params.force { cmd.args(["branch", "-D", &n]); } else { cmd.args(["branch", "-d", &n]); }
            }
        ...
        if output.status.success() {
            ...
            Err(anyhow::anyhow!("Branch operation failed: {}", String::from_utf8_lossy(&output.stderr)))
        }
    }
}
