...
pub async fn ensure_documents_table(connection: &Connection, embedding_dim: usize) -> Result<()> {
    ...
    connection
        .create_table(DOCUMENTS_TABLE, Box::new(batches) as Box<dyn arrow_array::RecordBatchReader + Send>)
        .execute()
        .await
...
pub async fn ensure_document_metadata_table(connection: &Connection) -> Result<()> {
    ...
    connection
        .create_table(DOCUMENT_METADATA_TABLE, Box::new(batches) as Box<dyn arrow_array::RecordBatchReader + Send>)
        .execute()
        .await
