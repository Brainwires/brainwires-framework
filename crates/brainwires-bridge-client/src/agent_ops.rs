...
impl BridgeClient {
    ...
    ) -> Result<AgentResult, BridgeClientError> {
    ...
    pub async fn list_agents(&mut self) -> Result<Vec<AgentInfo>, BridgeClientError> {
        let result = self.call_tool("agent_list", json!({})).await?;
        parse_agent_list(&result)
    }

    pub async fn stop_agent(&mut self, agent_id: &str) -> Result<(), BridgeClientError> {
    ...
    pub async fn get_agent_status(
        &mut self,
        agent_id: &str,
    ) -> Result<AgentInfo, BridgeClientError> {
        let result = self
...
fn extract_agent_id(result: &serde_json::Value) -> Result<String, BridgeClientError> {
    ...
    Err(BridgeClientError::Protocol(
...
fn parse_agent_list(result: &serde_json::Value) -> Result<Vec<AgentInfo>, BridgeClientError> {
    let text = extract_text_content(result).unwrap_or_default();
