...
brainwires-storage = { version = "0.5", default-features = false, features = ["wasm"] }

# With Qdrant backend (in addition to LanceDB)
brainwires-storage = { version = "0.5", features = ["qdrant-backend"] }

# PostgreSQL as primary backend
brainwires-storage = { version = "0.5", features = ["postgres-backend"] }

# MySQL / MariaDB backend
brainwires-storage = { version = "0.5", features = ["mysql-backend"] }

# SurrealDB backend (native vector search)
brainwires-storage = { version = "0.5", features = ["surrealdb-backend"] }

# NornicDB with all transports
brainwires-storage = { version = "0.5", features = ["nornicdb-full"] }
```
...
# Via facade
[dependencies]
brainwires = { version = "0.5", features = ["storage"] }
