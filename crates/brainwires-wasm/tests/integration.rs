...
use brainwires_wasm::{validate_message, validate_tool, serialize_history, version};
...
fn version_is_valid_semver() {
    ...
    assert_eq!(parts.len(), 3, "version should have 3 semver components: {v}");
    for part in &parts {
        part.parse::<u32>()
...
fn validate_message_valid_user_text() {
    ...
    assert!(result.is_ok(), "valid user message should succeed: {result:?}");
...
fn validate_message_valid_assistant() {
    ...
    let result = validate_message(json);
    assert!(result.is_ok(), "valid assistant message should succeed: {result:?}");
}
...
fn validate_message_valid_system() {
    ...
    let result = validate_message(json);
    assert!(result.is_ok(), "valid system message should succeed: {result:?}");
}
...
fn validate_message_with_optional_name() {
    ...
    let result = validate_message(json);
    assert!(result.is_ok(), "message with name field should succeed: {result:?}");
    let out = result.unwrap();
...
fn validate_message_with_metadata() {
    ...
    let result = validate_message(json);
    assert!(result.is_ok(), "message with metadata should succeed: {result:?}");
}
...
fn validate_tool_valid_minimal() {
    ...
    assert!(result.is_ok(), "empty object (all defaults) should succeed: {result:?}");
}
...
fn validate_tool_valid_full() {
    ...
    let result = validate_tool(json);
    assert!(result.is_ok(), "full tool definition should succeed: {result:?}");
    let out = result.unwrap();
    assert!(out.contains("calculator"), "output should contain tool name");
    assert!(out.contains("Performs arithmetic"), "output should contain description");
}
...
fn validate_tool_with_name_only() {
    ...
    let result = validate_tool(json);
    assert!(result.is_ok(), "tool with only name should succeed: {result:?}");
}
...
fn serialize_history_valid_array() {
    ...
    assert!(result.is_ok(), "valid message array should succeed: {result:?}");
    let out = result.unwrap();
    // Output should be valid JSON
    let _parsed: serde_json::Value = serde_json::from_str(&out)
        .expect("serialize_history output should be valid JSON");
}
...
fn serialize_history_single_message() {
    let json = r#"[{"role":"user","content":"Hello"}]"#;
    let result = serialize_history(json);
    assert!(result.is_ok(), "single message array should succeed: {result:?}");
}
...
fn serialize_history_multiple_roles() {
    ...
    let result = serialize_history(json);
    assert!(result.is_ok(), "multi-role conversation should succeed: {result:?}");
}
