...
//!
//! ```rust,ignore
//! use brainwires_agents::runtime::{AgentRuntime, run_agent_loop, AgentExecutionResult};
//! use brainwires_agents::{CommunicationHub, FileLockManager};
//!
//! struct MyAgent { /* ... */ }
//!
...
use brainwires_core::{ChatResponse, ToolResult, ToolUse};
...
pub async fn run_agent_loop(
    ...
    loop {
        ...
        for tool_use in &tool_use_requests {
            ...
            if let Some((path, lock_type)) = agent.get_lock_requirement(tool_use) {
                ...
                match lock_manager.acquire_lock(&agent_id, &path, lock_type).await {
                    Ok(_guard) => {
                        match agent.execute_tool(tool_use).await {
                            Ok(result) => agent.on_tool_result(tool_use, &result).await,
                            Err(e) => {
