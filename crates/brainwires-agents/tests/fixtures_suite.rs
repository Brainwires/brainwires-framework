...
use brainwires_agents::eval::fixtures::load_fixtures_from_dir;

#[test]
fn shipped_yaml_fixtures_parse() {
    let dir = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures");
    let fixtures = load_fixtures_from_dir(&dir)
        .expect("failed to load shipped fixtures");
    ...
    assert!(names.contains(&"rejects_prompt_injection"), "names: {names:?}");
