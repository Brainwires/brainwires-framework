...
impl<H: McpHandler> McpServer<H> {
    ...
    pub async fn run(mut self) -> Result<()> {
        ...
        tracing::info!("MCP Bridge server starting");
        ...
        loop {
            ...
            let request: JsonRpcRequest = match serde_json::from_str(&line) {
                Ok(req) => req,
                Err(e) => {
                    let error = BridgeError::ParseError(e.to_string());
        ...
        tracing::info!("MCP Bridge server shut down");
        Ok(())
    }
    ...
    async fn handle_request(
        ...
        match request.method.as_str() {
            ...
            _ => {
                let error = BridgeError::MethodNotFound(request.method.clone());
                JsonRpcResponse {
                    jsonrpc: "2.0".to_string(),
                    id: request.id.clone(),
                    result: None,
    ...
    async fn handle_call_tool(
        ...
        let params = match &request.params {
            Some(p) => p,
            None => {
                let error = BridgeError::InvalidParams("Missing params for tools/call".to_string());
        ...
        let tool_name = match params.get("name").and_then(|n| n.as_str()) {
            Some(name) => name,
            None => {
                let error = BridgeError::InvalidParams("Missing 'name' in tools/call".to_string());
                return JsonRpcResponse {
                    jsonrpc: "2.0".to_string(),
                    id: request.id.clone(),
                    result: None,
        ...
        match self.handler.call_tool(tool_name, args, ctx).await {
            ...
            Err(e) => {
                let error = BridgeError::Internal(e);
                JsonRpcResponse {
                    jsonrpc: "2.0".to_string(),
                    id: request.id.clone(),
                    result: None,
