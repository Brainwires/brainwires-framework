...
//! for small matrices (up to ~20×20), using pure ndarray with no
...
/// Compute `log det(A_{S∪{c}})` incrementally given `log det(A_S)` and the
...
/// candidate c gives an (m+1)×(m+1) matrix:
///
/// ```text
/// A' = [ A_S    a ]
///      [ aᵀ    d ]
/// ```
///
/// where `a = L_S⁻¹ · cross` (cross = column of kernel values between S and c),
/// and `d = kernel(c,c)`.
///
/// `log det(A') = log det(A_S) + log(d - ‖a‖²)`   (the Schur complement).
///
...
mod tests {
    ...
    fn test_cholesky_extend_matches_full() {
        // Build a 3×3 PD matrix, compute Cholesky of 2×2 sub, then extend
        let full = array![
            [4.0_f32, 2.0, 1.0],
            [2.0, 5.0, 3.0],
            [1.0, 3.0, 6.0]
