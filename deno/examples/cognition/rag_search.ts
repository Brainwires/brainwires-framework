...
import { DEFAULT_LIMIT, DEFAULT_MAX_FILE_SIZE, DEFAULT_MIN_SCORE } from "@brainwires/cognition";
...
class MockRagClient implements RagClient {
  ...
  async advancedSearch(req: import("@brainwires/cognition").AdvancedSearchRequest): Promise<QueryResponse> {
    return this.queryCodebase({ query: req.query, limit: req.limit, minScore: req.minScore });
  }

  async searchGitHistory(req: import("@brainwires/cognition").SearchGitHistoryRequest) {
