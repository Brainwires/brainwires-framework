...
export class BrainwiresRelayProvider implements Provider {
    ...
    const content: string | ContentBlock[] = tool_calls.length === 0
      ...
      : (() => {
        ...
        if (full_text.length > 0) {
          blocks.push({ type: "text", text: full_text });
        }
  ...
  async *streamChat(
    ...
    if (options.system !== undefined) {
      request_body.systemPrompt = options.system;
    }
      ...
      switch (eventType) {
        ...
        case "toolCall": {
          try {
            ...
            const call_id = typeof parsed.callId === "string"
              ? parsed.callId
              : "";
            const response_id = typeof parsed.responseId === "string"
              ? parsed.responseId
              : "";
            const tool_name = typeof parsed.toolName === "string"
              ? parsed.toolName
              : "";
            const server = typeof parsed.server === "string"
              ? parsed.server
              : "";
            const chat_id = typeof parsed.chatId === "string"
              ? parsed.chatId
              : undefined;
...
export function buildRequestParts(messages: Message[]): RequestParts {
  ...
  if (func_output !== null) {
    ...
    const response_id_from_metadata = md && typeof md.response_id === "string"
      ? md.response_id
      : null;

    const history = messagesToHistory(
      messages.slice(0, Math.max(0, messages.length - 2)),
    );
    return {
      current_content: "",
