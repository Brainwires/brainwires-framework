...
Deno.test("file path condition", () => {
  ...
  assert(
    policyMatches(policy, policyRequestForFile(".env.local", "read_file")),
  );
  assert(
    !policyMatches(policy, policyRequestForFile("src/main.rs", "read_file")),
  );
});
...
Deno.test("policy engine evaluation", () => {
  ...
  // Should be allowed
  const request2 = {
    ...policyRequestForFile("src/main.rs", "read_file"),
    tool_category: "FileRead" as const,
  };
...
Deno.test("NOT condition", () => {
  const cond: PolicyCondition = {
    type: "not",
    condition: { type: "tool", name: "read_file" },
  };
  const policy = createPolicy("test", {
    conditions: [cond],
    action: PolicyActions.Deny,
