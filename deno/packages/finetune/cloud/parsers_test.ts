...
Deno.test("OpenAI parses running + succeeded", () => {
  const running = OpenAiFineTune.parseJobStatus("running", {
    trained_tokens: 1000,
  });
  assertEquals(running.status, "running");

  const ok = OpenAiFineTune.parseJobStatus("succeeded", {
    fine_tuned_model: "ft:gpt-4o-mini:abc",
  });
  assertEquals(ok.status, "succeeded");
  if (ok.status === "succeeded") {
    assertEquals(ok.model_id, "ft:gpt-4o-mini:abc");
  }
});

Deno.test("OpenAI parses failed", () => {
  const failed = OpenAiFineTune.parseJobStatus("failed", {
...
Deno.test("Together parses completed with output_name", () => {
  const ok = TogetherFineTune.parseJobStatus("completed", {
    output_name: "org/ft-123",
  });
  assertEquals(ok.status, "succeeded");
...
Deno.test("Fireworks parses JOB_STATE_COMPLETED", () => {
  const ok = FireworksFineTune.parseJobStatus("JOB_STATE_COMPLETED", {
    model: "accounts/a/models/b",
  });
  assertEquals(ok.status, "succeeded");
});
