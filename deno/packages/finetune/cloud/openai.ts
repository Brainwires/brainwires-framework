...
export class OpenAiFineTune implements FineTuneProvider {
  ...
  static parseJobStatus(
    ...
    switch (status_str) {
      ...
      case "running": {
        const step = typeof body.trained_tokens === "number"
          ? body.trained_tokens
          : 0;
        return {
          status: "running",
  ...
  async uploadDataset(
    ...
    const id = body.id;
    if (typeof id !== "string") {
      throw TrainingError.upload("Missing file ID in response");
    }
  ...
  async createJob(config: CloudFineTuneConfig): Promise<TrainingJobId> {
    ...
    const id = resBody.id;
    if (typeof id !== "string") {
      throw TrainingError.provider("Missing job ID in response");
    }
  ...
  async getJobStatus(job_id: TrainingJobId): Promise<TrainingJobStatus> {
    ...
    const status_str = typeof body.status === "string"
      ? body.status
      : "unknown";
