...
export function regressionFault(
  ...
  const suggested =
    `Fix regression in eval case '${case_name}': success rate dropped from ` +
    `${(previous_rate * 100).toFixed(0)}% to ${
      (current_rate * 100).toFixed(0)
    }% ` +
    `(drop: ${
      (drop * 100).toFixed(0)
    }%). Investigate recent changes and restore reliability.`;
  return {
    case_name,
    category,
...
export function newCapabilityFault(
  ...
  const suggested =
    `Record baseline for newly-observed eval case '${case_name}' ` +
  ...
  for (const [case_name, stats] of suiteResult.stats) {
    ...
    if (n_failures > 0 && ci_width > flaky_ci_threshold) {
      const suggested = `Stabilize flaky eval case '${case_name}' ` +
        `(mean success: ${(success_rate * 100).toFixed(0)}%, CI width: ${
          ci_width.toFixed(2)
        }). ` +
    ...
    if (baseline === null && regressionSuite !== null && success_rate >= 0.8) {
      reports.push(
        newCapabilityFault(
          case_name,
          case_name,
          `New eval case '${case_name}' achieving ${
            (success_rate * 100).toFixed(0)
          }% success — baseline not yet recorded`,
          success_rate,
          n_failures,
          n_trials,
        ),
