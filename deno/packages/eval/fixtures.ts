...
export function evaluate(
  ...
  if (expected.tool_sequence.length > 0) {
    ...
    if (
      ...
      return `tool_sequence mismatch: expected ${JSON.stringify(a)}, got ${
        JSON.stringify(b)
      }`;
    }
  }
  for (const a of expected.assertions) {
    if (a.contains !== undefined && !outcome.output_text.includes(a.contains)) {
      return `output_text missing expected substring: ${
    ...
    if (
      ...
      return `expected tool \`${a.tool_called}\` to be called; got ${
        JSON.stringify(outcome.tool_sequence)
      }`;
    }
    if (a.finish_reason !== undefined) {
      const got = outcome.finish_reason ?? "";
      if (got !== a.finish_reason) {
        return `finish_reason mismatch: expected ${
          JSON.stringify(a.finish_reason)
