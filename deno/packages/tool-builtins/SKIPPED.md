...
These modules are part of `brainwires-tools` (Rust) but are not available in
`@brainwires/tools` (Deno). Each skip has a specific technical reason below.
Callers that need these capabilities should run the Rust framework and
communicate with Deno agents over `@brainwires/network` / `@brainwires/a2a`.
...
Sandboxed tool execution that delegates to `brainwires-sandbox` (Docker/Podman).
