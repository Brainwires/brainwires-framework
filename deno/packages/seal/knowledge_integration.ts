...
export function validateIntegrationConfig(c: IntegrationConfig): void {
  if (
    ...
    throw new Error(
      "min_seal_quality_for_bks_boost must be between 0.0 and 1.0",
    );
  }
...
export class SealKnowledgeCoordinator {
  ...
  async getBksContext(query: string): Promise<string | undefined> {
    if (!this.integrationConfig.enabled) return undefined;
    const truths = await this.bks_cache.getMatchingTruthsWithScores(
      query,
      0.5,
      5,
    );
  ...
  harmonizeConfidence(
    ...
    return totalWeight > 0
      ? Math.min(weightedSum / totalWeight, 1.0)
      : seal_quality;
  }
  ...
  async checkAndPromotePattern(
    ...
    if (
      pattern.reliability() < this.integrationConfig.pattern_promotion_threshold
    ) {
      return undefined;
    }
