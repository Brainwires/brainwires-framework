...
pub(crate) fn stt_transcribe_sync(
    ...
    with_stt(handle, |stt| {
        let native_audio = brainwires_audio::AudioBuffer::from(audio);
        let native_opts = brainwires_audio::SttOptions::from(options);
        runtime()
...
pub(crate) fn audio_play_sync(
    ...
    let playback = CpalPlayback::new();
    let native_buf = brainwires_audio::AudioBuffer::from(buffer);
    runtime()
