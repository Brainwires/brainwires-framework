...
impl GatewayClient {
    ...
    async fn send_event(&mut self, event: &ChannelEvent) -> Result<()> {
    ...
    /// Main run loop: forwards Signal events to the gateway and relays
    /// outbound messages from the gateway back to Signal.
    pub async fn run(
        mut self,
                        ...
                        Ok(Some(text)) => {
                            if let Ok(msg) = serde_json::from_str::<ChannelMessage>(&text) {
                                if let Err(e) = channel.send_message(&msg.conversation.clone(), &msg).await {
                                    tracing::error!("Failed to send message to Signal: {e}");
                                }
                            } else {
