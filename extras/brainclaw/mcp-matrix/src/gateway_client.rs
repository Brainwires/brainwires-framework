...
impl GatewayClient {
    ...
    /// Main run loop: forwards Matrix events to the gateway and sends
    /// outbound messages from the gateway back to Matrix.
    pub async fn run(
        mut self,
                            ...
                            match serde_json::from_str::<ChannelMessage>(&text) {
                                Ok(msg) => {
                                    if let Err(e) = channel.send_message(&msg.conversation, &msg).await {
                                        tracing::error!("Failed to send message to Matrix: {}", e);
                                    }
                                }
                                Err(e) => {
