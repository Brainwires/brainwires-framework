...
impl GatewayClient {
    ...
    /// Main run loop: forwards WhatsApp events to the gateway and sends
    /// outbound messages from the gateway back to WhatsApp.
    pub async fn run(
        mut self,
                    ...
                    match result {
                        Ok(Some(text)) => {
                            match serde_json::from_str::<ChannelMessage>(&text) {
                                Ok(msg) => {
                                    if let Err(e) = channel.send_message(&msg.conversation, &msg).await {
                                        tracing::error!("Failed to send message to WhatsApp: {}", e);
                                    }
                                }
                                Err(e) => {
