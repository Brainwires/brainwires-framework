...
impl GatewayClient {
    ...
    /// Main run loop: forwards Discord events to the gateway and relays
    /// outbound messages from the gateway back through Discord.
    pub async fn run(
        mut self,
        mut event_rx: mpsc::Receiver<ChannelEvent>,
        discord_channel: Arc<DiscordChannel>,
    ) -> Result<()> {
        tracing::info!("Gateway client run loop started");

        loop {
            tokio::select! {
                // Forward inbound Discord events to gateway
                    ...
                    match result {
                        Ok(Some(text)) => {
                            match serde_json::from_str::<ChannelMessage>(&text) {
                                Ok(msg) => {
                                    if let Err(e) = discord_channel.send_message(&msg.conversation, &msg).await {
...
mod tests {
    ...
    fn build_handshake_message() {
        let caps = ChannelCapabilities::RICH_TEXT | ChannelCapabilities::EMBEDS;
        let hs = build_handshake("my-secret-token", caps);

        assert_eq!(hs.channel_type, "discord");
        assert_eq!(hs.auth_token, "my-secret-token");
        assert!(hs.capabilities.contains(ChannelCapabilities::RICH_TEXT));
        assert!(hs.capabilities.contains(ChannelCapabilities::EMBEDS));
