...
impl GatewayClient {
    ...
    /// Main run loop: forwards Slack events to the gateway and relays
    /// outbound messages from the gateway back through Slack.
    pub async fn run(
        mut self,
        ...
        loop {
            tokio::select! {
                // Forward inbound Slack events to gateway
                    ...
                    match result {
                        Ok(Some(text)) => {
                            match serde_json::from_str::<ChannelMessage>(&text) {
                                Ok(msg) => {
                                    if let Err(e) = slack_channel.send_message(&msg.conversation, &msg).await {
...
mod tests {
    ...
    fn build_handshake_message() {
        ...
        assert_eq!(hs.channel_type, "slack");
    ...
    fn handshake_serialization() {
        ...
        assert_eq!(parsed.channel_type, "slack");
